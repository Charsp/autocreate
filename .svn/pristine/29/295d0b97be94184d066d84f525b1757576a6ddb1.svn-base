...
import com.common.mybatis.mapper.builder.SqlBuilder;
...
public class CacheInterceptor implements Interceptor {
	...
	private static String defaultPageSqlId = ".*select_[a-z]*"; // 需要拦截的ID(正则匹配)
	...
	private CacheKey createCacheKey(MappedStatement ms, Object parameterObject, RowBounds rowBounds,
		...
		if (null == boundSql.getSql() || "".equals(boundSql.getSql())) {
			String id = ms.getId();
			id = id.substring(id.lastIndexOf(".") + 1).substring(0, id.lastIndexOf("_"));
			String newSql = null;
			try {
				if ("select".equals(id)) {
